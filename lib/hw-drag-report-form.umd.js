...
function correctBounds(layout, bounds) {
  ...
  layout.map(function (item) {
    // 超出右侧，则贴着右侧
    if (item.x + item.w > bounds.cols) item.x = bounds.cols - item.w; // 超出左侧
...
function moveElement(layout, l, x, y, isUserAction, preventCollision) {
  ...
  var oldX = l.x;
  var oldY = l.y;
  var movingUp = y && l.y > y; // 这比扩展对象要快很多
  ...
  for (var i = 0, len = collisions.length; i < len; i++) {
    ...
    if (item.moved) continue; // 向上移动时，只需稍等一下即可使它感觉更加精确

    if (l.y > item.y && l.y - item.y > item.h / 4) continue; // 不需要移动静态选项
...
function compact(layout, verticalCompact) {
  ...
  var compareWith = getStatics(layout); // 按行和列检查这些选项

  var sorted = sortLayoutItemsByRowCol(layout);
  var out = Array(layout.length);
  sorted.map(function (item) {
    var row = item; // 不能移动静态元素

    if (!item.static) {
      row = compactItem(compareWith, row, verticalCompact); // 添加到compareWith数组

      compareWith.push(row);
    } // 添加到out数组以确保它们仍然以正确的顺序出现


    out[layout.indexOf(row)] = row; // 清除移动标志
...
function offsetXYFromParentOf(e) {
  var offsetParent = e.target.offsetParent || document.body; // ele.getBoundingClientRect() 返回元素的大小及其相对于视口的位置
...
/* harmony default export */ var hw_grid_itemvue_type_script_lang_js_ = ({
  ...
  methods: {
    ...
    createStyle: function createStyle() {
      ...
      if (this.isDragging) {
        pos.top = this.dragging.top; // 添加rtl支持
    ...
    handleResize: function handleResize(event) {
      ...
      var position = getControlPosition(event); // 获取事件的当前拖动点作为偏移量
    ...
    handleDrag: function handleDrag(event) {
      ...
      var position = getControlPosition(event); // 获取事件的当前拖动点用作偏移量
        ...
        case 'dragstart':
          {
            ...
            this.previousY = this.innerY; // 获取父元素的位置集合
    ...
    calcPosition: function calcPosition(x, y, w, h) {
      var colWidth = this.calcColWidth(); // 添加rtl支持
...
function findOrGenerateResponsiveLayout(orgLayout, layouts, breakpoints, breakpoint, lastBreakpoint, cols, verticalCompact) {
  // 如果存在，则返回
  if (layouts[breakpoint]) return cloneLayout(layouts[breakpoint]); // 查找或生成下一个布局
...
/* harmony default export */ var hw_grid_layoutvue_type_script_lang_js_ = ({
  ...
  created: function created() {
    ...
    self.eventBus = self._provided.eventBus; // $on： 监听当前实例的自定义事件
  ...
  methods: {
    ...
    resizeEvent: function resizeEvent(eventName, id, x, y, h, w) {
      ...
      var l = getLayoutItem(this.layout, id); // 当布局项返回空对象时
      ...
      if (this.preventCollision) {
        ...
        hasCollisions = collisions.length > 0; // 如果发生碰撞，需要调整占位元素
    ...
    responsiveGridLayout: function responsiveGridLayout() {
      ...
      var newCols = getColsFromBreakpoint(newBreakpoint, this.cols); // 在布局中保存实际布局
      ...
      var layout = findOrGenerateResponsiveLayout(this.originalLayout, this.layouts, this.breakpoints, newBreakpoint, this.lastBreakpoint, newCols, this.verticalCompact); // 保存新布局

      this.layouts[newBreakpoint] = layout; // 同步布局
    ...
    dragEvent: function dragEvent(eventName, id, x, y, h, w) {
      ...
      var l = getLayoutItem(this.layout, id); // 布局选项有时会返回空对象
